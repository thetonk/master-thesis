...
_non_numeric_columns = ["Src IP", "Dst IP", "Timestamp", "Label"]
_dataset_columns = [
...
def _prepare_numeric_columns(df: pd.DataFrame) -> pd.DataFrame:
    for column in df.columns:
        if column not in _non_numeric_columns and not pd.api.types.is_numeric_dtype(df[column]):
        ...
        for file in files:
            ...
            with pd.read_csv(csv_file_path, chunksize=1e+6, low_memory=False, delimiter=",") as csv_reader:
                for chunk in csv_reader:
                    ...
                    chunk = chunk.reindex(columns=_dataset_columns)
                    print("Chunk shape:",chunk.shape, "Datatypes:", chunk.dtypes)
...
def dataset_to_tensor(dataset_path, label_column) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    y_tensor = torch.tensor(y_df[label_column].astype('category').cat.codes.to_numpy(), dtype=torch.int64)
